...
export default function CCNManagementTab() {
  ...
  async function handleImportCCN(ccn: CCNReferentiel) {
    try {
      ...
      // Insérer la CCN dans la table ccn avec statut "importing"
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          status: 'importing',
          import_source: 'legifrance',
        } as any) as any;

      if (insertError) {
        console.error('Erreur insertion:', insertError);
        toast.error(`Erreur lors de l'import: ${insertError.message}`);
        return;
      }

      toast.success(
        `Import de la CCN ${ccn.idcc} démarré`,
        {
          description: `${ccn.titre} - L'import peut prendre quelques minutes`,
        }
      );
  ...
  const filteredDB = ccnInDB.filter(
    (ccn) =>
      ...
      ccn.titre.toLowerCase().includes(searchDB.toLowerCase())
  );

  const filteredAvailable = ccnAvailable.filter(
    (ccn) =>
      ccn.idcc.toLowerCase().includes(searchAvailable.toLowerCase()) ||
      ccn.titre.toLowerCase().includes(searchAvailable.toLowerCase())
  );
                ...
                <TableBody>
                  {filteredDB.map((ccn, index) => (
                    <TableRow key={`${ccn.idcc}-${index}`} className="hover:bg-green-50/50 transition-colors">
                      ...
                      <TableCell>
                        <div className="text-sm" title={ccn.titre}>
                          {ccn.titre}
                        </div>
                      </TableCell>
                      <TableCell>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredAvailable.map((ccn) => (
                    <TableRow key={ccn.id} className="hover:bg-blue-50/50 transition-colors">
                      <TableCell className="font-medium">
                        <span className="font-mono text-blue-600 font-semibold">{ccn.idcc}</span>
                      </TableCell>
                      <TableCell className="max-w-xs">
                        <div className="truncate text-sm" title={ccn.titre}>
                          {ccn.titre}
                        </div>
                        {ccn.brochure && (
